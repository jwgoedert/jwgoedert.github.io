...
(function() {
    ...
    _.last = function(array, n) {
        ...
        if (!Array.isArray(array) || n < 0) return [];
        if (n === undefined || typeof n !== "number" || n === array.length) return array[array.length - 1];
    ...
    _.each = function(collection, action) {
        if (Array.isArray(collection)) {
            for (var i = 0; i < collection.length; i++) {
                action(collection[i], i, collection); //higher order, always pas the value, 
                //the position then the full collection
            }
        }
        else {
            //it's an object
     ...
     *   3) Tis is the logical inverse if _.filter(), you must use _.filter() in your implementation
     * Examples:
    ...
    */
     ...
     */
    _.pluck = function(array, property) { //takes an array of objects and a property
        return _.map(array, function(value, i, array) {
    ...
    _.every = function(collection, test) {
        // loop thru the collection (one type of loop for Array, one type of loop for Object)
        ...
        for (var keys in collection) {
            if (test(collection[keys], keys, collection) === false) {
                return false;
            }
        }
        return true;
        ...
        // test every value, passing value, pos, collection
        // as soon as one fails, bails
        // if you get thru the full array and there is no failures, return true because all values passed
    ...
    _.some = function(collection, test) {
            ...
            for (var keys in collection) {
                if (test(collection[keys], keys, collection) === true) {
                    return true;
                }
            }
            return false;
   ...
   return seed;
     ...
     */
_.extend = function(obj1,...copyFromObjects){
  
    for(var i = 0 ; i <= copyFromObjects.length; i++){
        var copiedIndex= copyFromObjects[i];
    for(var keys in copiedIndex){
        obj1[keys]=copiedIndex[keys];
    }
    }return obj1;
